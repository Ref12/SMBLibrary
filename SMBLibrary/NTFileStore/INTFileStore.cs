...
namespace SMBLibrary
{
    ...
    /// </summary>
    public interface INTFileStore : INTFileStore<object>
    {
    ...
    public interface INTFileStore<THandle> : INTStreamStore<THandle>, INTControlStore<THandle>
    {

    }

    public interface INTControlStore<THandle> 
    {
        NTStatus LockFile(THandle handle, long byteOffset, long length, bool exclusiveLock);

        NTStatus UnlockFile(THandle handle, long byteOffset, long length);

        NTStatus QueryDirectory(out List<QueryDirectoryFileInformation> result, THandle handle, string fileName, FileInformationClass informationClass);

        NTStatus GetFileInformation(out FileInformation result, THandle handle, FileInformationClass informationClass);

        NTStatus SetFileInformation(THandle handle, FileInformation information);

        NTStatus GetFileSystemInformation(out FileSystemInformation result, FileSystemInformationClass informationClass);

        NTStatus SetFileSystemInformation(FileSystemInformation information);

        NTStatus GetSecurityInformation(out SecurityDescriptor result, THandle handle, SecurityInformation securityInformation);

        NTStatus SetSecurityInformation(THandle handle, SecurityInformation securityInformation, SecurityDescriptor securityDescriptor);
        ...
        /// STATUS_INVALID_HANDLE - The handle supplied is invalid.
        /// </returns>
        NTStatus NotifyChange(out object ioRequest, THandle handle, NotifyChangeFilter completionFilter, bool watchTree, int outputBufferSize, OnNotifyChangeCompleted onNotifyChangeCompleted, object context);

        NTStatus Cancel(object ioRequest);

        NTStatus DeviceIOControl(THandle handle, uint ctlCode, byte[] input, out byte[] output, int maxOutputLength);
    }

    public interface INTStreamStore<THandle>
    {
        NTStatus CreateFile(out THandle handle, out FileStatus fileStatus, string path, AccessMask desiredAccess, FileAttributes fileAttributes, ShareAccess shareAccess, CreateDisposition createDisposition, CreateOptions createOptions, SecurityContext securityContext);

        NTStatus CloseFile(THandle handle);

        NTStatus FlushFileBuffers(THandle handle);

        NTStatus ReadFile(out byte[] data, THandle handle, long offset, int maxCount);

        NTStatus WriteFile(out int numberOfBytesWritten, THandle handle, long offset, byte[] data);
    }
}
